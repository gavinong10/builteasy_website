 ...
 */
 ...
 /**
 * Thrive_Dash_Api_Sendreach_Params implements a collection that takes key-value pairs.
 *
 * You can access, add or remove an item with a key by using
 * {@link itemAt}, {@link add}, and {@link remove}.
 * To get the number of the items in the map, use {@link getCount}.
 * Thrive_Dash_Api_Sendreach_Params can also be used like a regular array as follows,
 * <pre>
 * $map[$key]=$value; // add a key-value pair
 * unset($map[$key]); // remove the value with the specified key
 * if(isset($map[$key])) // if the map contains the key
 ...
 *
 * @property boolean $readOnly Whether this map is read-only or not. Defaults to false.
 * @property Thrive_Dash_Api_Sendreach_ParamsIterator $iterator An iterator for traversing the items in the list.
 * @property integer $count The number of items in the map.
 * @property array $keys The key list.
 * 
 ...
 */
class Thrive_Dash_Api_Sendreach_Params extends Thrive_Dash_Api_Sendreach implements IteratorAggregate,ArrayAccess,Countable
{
	/**
	 ...
	 * @throws Thrive_Dash_Api_Sendreach_Exception If data is not null and neither an array nor an iterator.
	 */
	 ...
	 * @return Thrive_Dash_Api_Sendreach_ParamsIterator an iterator for traversing the items in the list.
	 */
	public function getIterator()
	{
		return new Thrive_Dash_Api_Sendreach_ParamsIterator($this->_data);
	}
	 ...
	 * @throws Thrive_Dash_Api_Sendreach_Exception if the map is read-only
	 */
	public function add($key, $value)
	{
		if (!$this->_readOnly) {
			...
			throw new Thrive_Dash_Api_Sendreach_Exception('The params map is read only.');
		}
	}
	 ...
	 *
	 * @throws Thrive_Dash_Api_Sendreach_Exception If data is neither an array nor an iterator.
	 */
	public function copyFrom($data)
	{
		if (is_array($data) || $data instanceof Traversable) {
			...
			if ($data instanceof Thrive_Dash_Api_Sendreach_Params) {
				$data = $data->_data;
			}	
			...
			throw new Thrive_Dash_Api_Sendreach_Exception('Params map data must be an array or an object implementing Traversable.');
		}	
	}

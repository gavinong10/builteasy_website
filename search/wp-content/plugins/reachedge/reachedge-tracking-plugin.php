...
function reachedge_tracking_plugin() {
  ...
  if (strlen($reachlocal_tracking_id) == strlen(constant('DEFAULT_CODE')) && $reachlocal_tracking_id != DEFAULT_CODE) {
    $output = '<script type="text/javascript">var rl_siteid = "' .  $reachlocal_tracking_id . '";</script>';
    echo $output;
    wp_enqueue_script( 'reachlocal_tracking_script', "//cdn.rlets.com/capture_static/mms/mms.js#reachlocal_async");
  }
}

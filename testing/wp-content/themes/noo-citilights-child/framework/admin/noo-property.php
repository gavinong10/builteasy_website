	...
	class NooProperty{
		...
		public static function enqueue_gmap_js( $load_map_data = false ,$ids) {
			...
			$nooGmapL10n = array(
				...
				'zoom'=>self::get_google_map_option('zoom',10),
...
<input type="text" name="keyword" value="<?php echo $_GET['keyword'];?>" placeholder="Keyword" class="keyword-search">
		...
		public static function get_property_summary( $args = '' ) {
			...
			if( !empty( $bedrooms ) ) {
				$html[] = '<div class="bedrooms"><span>' . esc_html($bedrooms) . '</span></div>';
			}
			if( !empty( $bathrooms ) ) {
				$html[] = '<div class="bathrooms"><span>' . esc_html($bathrooms) . '</span></div>';
			}
			if( !empty( $parking ) ) {

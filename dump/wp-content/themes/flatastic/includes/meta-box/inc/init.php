...
add_action( 'admin_init', 'mad_rwmb_register_meta_boxes' );
...
function mad_rwmb_register_meta_boxes() {
	$mad_meta_boxes = apply_filters( 'rwmb_meta_boxes', array() );

	foreach ( $mad_meta_boxes as $meta_box ) {
		new RW_Meta_Box( $meta_box );
	}
}

add_action( 'edit_page_form', 'mad_rwmb_fix_page_template' );
 ...
 * @since 4.3.10
 */
function mad_rwmb_fix_page_template( $post )
{
